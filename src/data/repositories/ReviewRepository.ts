  ...
  implements IReviewRepository
{
  ...
  async getReviewsByApartment(apartmentId: string): Promise<IReview[]> {
    ...
    `;
    const result = await this.pool.query(queryText, [apartmentId]);
    return result.rows;
  }
  ...
  async updateReview(
    ...
    const queryText = `UPDATE reviews SET ${setClause} WHERE id = $1 RETURNING *`;
    const result = await this.pool.query(queryText, [id, ...values]);
    return result.rows[0] || null;
  }
